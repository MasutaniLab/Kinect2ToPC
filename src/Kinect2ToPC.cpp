...
RTC::ReturnCode_t Kinect2ToPC::onInitialize() {
	...
	bindParameter("trimming_X", m_trimming_X, "10");  // プラス 単位m
...
RTC::ReturnCode_t Kinect2ToPC::onActivated(RTC::UniqueId ec_id) {
	...
	try {
		...
		m_pc.point_step = 16;
		m_pc.is_dense = false;
		boost::function<void(const pcl::PointCloud<PointT>::ConstPtr&)>
		f = boost::bind(&Kinect2ToPC::cloud_cb, this, _1);
...
void Kinect2ToPC::cloud_cb(const pcl::PointCloud<PointT>::ConstPtr &cloud) {
	...
	for (unsigned int i = 0; i < cloud->points.size(); i++) {
		if (m_trimming_X > -cloud->points[i].x &&
			-cloud->points[i].x > -m_R_trimming_X &&
			m_trimming_Y > -cloud->points[i].y &&
			-cloud->points[i].y > -m_R_trimming_Y &&
			m_trimming_Z > -cloud->points[i].z &&
			-cloud->points[i].z > -m_R_trimming_Z) {
